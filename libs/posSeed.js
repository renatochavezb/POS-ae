...
export async function seedPosMarketingAgencyIfEmpty() {
  for (const member of INITIAL_MARKETING_AGENCIES) {
    await PosMarketingAgency.updateOne(
      { agencyCode: member.id },
      {
        $set: {
          name: member.name,
          role: member.role,
